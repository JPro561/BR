...
export type Stmt =
  | { kind: "VarDecl"; name: string; value: number | string }
  | { kind: "Print"; valueType: "IDENT" | "NUMBER" | "STRING"; value: string | number };
...
export function parser(tokens: Token[]) {
    ...
    while (pos < tokens.length) {
        ...
        if (token.type == "VAR") {
            ...
            if (!next) throw new Error("Fim inesperado do código");
            if (next.type != "IDENT") {
                ...
                if (nextnext.type != "EQUAL") {
                    ...
                    if (nextnextnext == undefined) break;
